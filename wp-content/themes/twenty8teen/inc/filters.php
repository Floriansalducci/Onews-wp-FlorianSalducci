...
function twenty8teen_post_classes( $classes, $class, $post_id ) {
	$classes[] = 'entry';
	if ( is_page() ) {
		if ( $i = array_search( 'hentry', $classes ) ) {
			unset( $classes[$i] );
		}
	}
 ...
 */
function twenty8teen_nav_add_widget_classes( $template, $class ) {
...
function twenty8teen_attachment( $p ) {
	...
	if ( ! wp_attachment_is( 'video', $post ) && ! wp_attachment_is( 'audio', $post ) ) {
		...
		$p .= wp_get_attachment_link( $post->ID, 'large', false, true);
		$p .= '</p>';
	}
	$p .= $post->post_excerpt ? ( '<p>'. wptexturize( $post->post_excerpt ) . '</p>' ) : '';
	return $p;
}
...
function twenty8teen_mce_style_formats( $init_array ) {
	...
	$style_formats = array_key_exists( 'style_formats', $init_array ) ?
		wp_json_decode( $init_array['style_formats'] ) : array();
	$text = $width = $border = $other = array();
		...
		if ( empty( $html ) ) {
			...
			if ( 'none' !== $opt ) {
				$imgattr = twenty8teen_add_gradient( get_permalink( $post_id ), array(
					'style' => '',
					'class' => 'wp-post-image wrapped-media-size-' . $size,
...
function twenty8teen_archive_in_tableview( $area_class, $area ) {
	if ( 'main' == $area && ! is_singular() ) {
		...
		if ( get_theme_mod( 'start_in_tableview', $default['start_in_tableview'] ) ) {
			...
			if ( $i = array_search( 'table-view', $classes ) ) {
				unset( $classes[$i] );
...
function twenty8teen_nav_menu_start_el( $item_output, $item, $depth, $args ) {
	...
	if ( $classes && in_array( 'menu-item-has-children', $classes ) ||
		...
		$item_output .= '<input type="checkbox" id="sub' . $item->ID
			. '" tabindex="-1"><label for="sub' . $item->ID . '"></label>';
	}
	return $item_output;
}
...
function twenty8teen_page_menu_start_el( $item_output, $page, $depth, $args ) {
	if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
		$item_output .= '<input type="checkbox" id="sub' . $page->ID
			. '" tabindex="-1"><label for="sub' . $page->ID . '"></label>';
	}
	return $item_output;
}
...
class Twenty8teen_Walker_Page extends Walker_Page {
	public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		$atts = apply_filters( 'page_menu_link_attributes', $atts, $page, $args, $depth );
		$attributes = '';
